...
class _StripePaymentWebState extends State<StripePaymentWeb> {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onNavigationRequest: (request) {
           ...
           if(status == null){
             ApiWrapper.showToastMessage("Wait");
           } else if(status == "success"){
              Get.back(result: uri.queryParameters["Transaction_id"]);
              ApiWrapper.showToastMessage("Payment Done Successfully");
            } else {
          ...
          onPageFinished: (String url) {
            // readJS();
            print("URL L L L L > > > > ${Config.base_url}/stripe/index.php?name=${widget.paymentCard.name}&email=${widget.paymentCard.email}&cardno=${widget.paymentCard.number}&cvc=${widget.paymentCard.cvv}&amt=${widget.paymentCard.amount}&mm=${widget.paymentCard.month}&yyyy=${widget.paymentCard.year}");
          },
          onProgress: (val) {
            progress = val;
          },
  ...
  String get initialUrl =>
      '${Config.base_url}stripe/index.php?name=${widget.paymentCard.name}&email=${widget.paymentCard.email}&cardno=${widget.paymentCard.number}&cvc=${widget.paymentCard.cvv}&amt=${widget.paymentCard.amount}&mm=${widget.paymentCard.month}&yyyy=${widget.paymentCard.year}';
  @override
  Widget build(BuildContext context) {
    if (_scaffoldKey.currentState == null) {
      return WillPopScope(
